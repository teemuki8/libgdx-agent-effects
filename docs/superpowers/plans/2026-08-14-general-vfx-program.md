...
- [x] **Step 1: Write dependency-boundary checks**
...
- [x] **Step 2: Run the boundary check and observe missing modules**

Run: `./gradlew verifyModuleBoundaries`

Expected: FAIL because `effects-import` is not included yet.

- [x] **Step 3: Add modules, publication metadata, locks, package docs, and ADR**
...
- [x] **Step 4: Verify module structure**
...
- [x] **Step 1: Write failing immutable-model and hard-limit tests**
...
- [x] **Step 2: Run focused core tests and confirm missing types**
...
- [x] **Step 3: Implement the smallest closed contracts**
...
- [x] **Step 4: Run focused core verification**
...
- [x] **Step 1: Execute every checked step in the importer-slice plan**
...
- [x] **Step 2: Verify the complete importer slice**
...
- [x] **Step 1: Write lifecycle, fixed-step, capacity, and close tests**
...
- [x] **Step 2: Run runtime tests and confirm missing API failure**

Run: `./gradlew :effects-runtime:test --tests '*EffectRuntimeTest'`

Expected: FAIL at compilation.

- [x] **Step 3: Implement explicit stepping without threads or timers**
...
- [x] **Step 4: Verify runtime and core**
...
- [x] **Step 1: Write real-GL material and host-state tests**
...
- [x] **Step 1: Write sampling, expiry, sharp-turn, and capacity tests**
...
- [x] **Step 3: Implement the bounded ring and immutable snapshot**

Use fixed-capacity arrays or a fixed-capacity deque; never allocate per frame after construction.

- [x] **Step 4: Write and pass the Xvfb ribbon pixel test**
...
- [x] **Step 1: Write endpoint, seed, regeneration, zero-length, and cap tests**

- [x] **Step 2: Run and observe missing types**

Run: `./gradlew :effects-runtime:test --tests '*BeamAndLightningTest'`

Expected: FAIL at compilation.

- [x] **Step 3: Implement deterministic segment generation**
...
- [x] **Step 4: Qualify the renderer under Xvfb**
...
- [x] **Step 1: Write emission, burst, fixed-step, expiry, capacity, and seed tests**
...
- [x] **Step 3: Implement allocation-free steady-state simulation**
...
- [x] **Step 4: Qualify sprite and point-sprite rendering**
...
- [x] **Step 1: Write capability-selection and state-bound tests**
...
- [x] **Step 2: Run and observe missing backend failure**

Run: `xvfb-run -a ./gradlew :effects-libgdx:test --tests '*GpuParticleInstanceTest' --tests '*ParticleBackendSelectorTest'`

Expected: FAIL at compilation.

- [x] **Step 3: Implement two RGBA state framebuffers with explicit swap**
...
- [x] **Step 4: Run the GL3 fixture and forced-fallback tests**
...
- [x] **Step 1: Write lifetime, order, capacity, and transform tests**
...
- [x] **Step 3: Implement bounded decal lifetime state**

- [x] **Step 4: Pass 2D and 3D Xvfb rendering/state-restoration tests**
...
- [x] **Step 1: Write graph cycle, unknown-input, ordering, and bound tests**
...
- [x] **Step 3: Implement immutable DAG validation and stable topological ordering**

- [x] **Step 4: Implement render-thread framebuffer pooling and distortion composition**

- [x] **Step 5: Qualify multi-input rendering under Xvfb**
...
- [x] **Step 1: Write known-field, unknown-field, bound, and approximation tests**

- [x] **Step 2: Run and observe missing importer failure**

Run: `./gradlew :effects-import:test --tests '*LibgdxParticleImporterTest' --tests '*FlameParticleImporterTest'`

Expected: FAIL at compilation.

- [x] **Step 3: Implement content parsers without instantiating libGDX runtime classes**

- [x] **Step 4: Verify import module and Javadocs**
...
- [x] **Step 1: Write JSON round-trip, unknown-field, version, and catalog-schema tests**

- [x] **Step 2: Run protocol and MCP tests and confirm failure**

Run: `./gradlew :effects-protocol:test :effects-mcp:test`

Expected: FAIL until all closed variants and handlers are registered.

- [x] **Step 3: Add bounded registry/backend operations and exact schemas**
...
- [x] **Step 4: Verify protocol and MCP gates**
...
- [x] **Step 1: Write the fixture test before adding the showcase scene**
...
- [x] **Step 2: Run the fixture and confirm missing scene failure**

Run: `.agents/skills/libgdx-agent-effects-dev/scripts/verify.sh fixture`

Expected: FAIL because `GeneralVfxScene` and its registered effects do not exist.

- [x] **Step 3: Implement the scene and Wings integration example**
...
- [x] **Step 5: Audit the final diff and commit**
