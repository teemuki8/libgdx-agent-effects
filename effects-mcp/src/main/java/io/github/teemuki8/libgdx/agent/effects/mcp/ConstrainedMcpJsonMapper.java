...
/**
 * {@link McpJsonMapper} with the same stream constraints as {@link EffectsJson}: 32-level
 * nesting, 16,384-character strings, and 128-digit numbers. Every other behavior of the
 * MCP SDK Jackson 3 mapper is retained by delegating to {@link JacksonMcpJsonMapper}.
 */
public class ConstrainedMcpJsonMapper implements McpJsonMapper {
    /** Matches the nesting bound used by {@link EffectsJson}. */
    private static final int MAX_NESTING_DEPTH = 32;
    /** Matches the string bound used by {@link EffectsJson}. */
    private static final int MAX_STRING_LENGTH = 16_384;
    /** Matches the number-token bound used by {@link EffectsJson}. */
     ...
     * array; streaming into a {@link BoundedOutputStream} lets the transport enforce the
     * outbound cap during serialization instead of after. The stream is not closed by this
     * method (Jackson closes only the generator it creates).
     */
