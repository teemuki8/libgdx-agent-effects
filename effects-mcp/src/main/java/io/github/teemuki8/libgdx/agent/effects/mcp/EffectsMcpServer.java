...
/** MCP SDK 2.0 server exposing only the five fixed effects tools over one stdio connection. */
public final class EffectsMcpServer implements AutoCloseable {
    /**
     * JSON-RPC implementation-defined error code returned when an outbound response message
     * would exceed {@link #MAX_RESPONSE_BYTES}. The transport cannot recover the oversized
     * message, so the client receives one bounded typed error instead of the partial
     * oversized frame; later requests still work.
     */
    static final int RESPONSE_LIMIT_EXCEEDED = -32001;

    /**
     * Maximum bytes of one serialized outbound response. The protocol module defines the
     * inbound request bound ({@code EffectsProtocol.MAX_REQUEST_BYTES}); the outbound cap
     * lives with the closed server, which in v0.1 emits no render artifacts.
     */
    static final int MAX_RESPONSE_BYTES = 8_388_608;

    private final StdioProvider transport;
    private final EffectsToolHandler handler;
    private final McpAsyncServer server;
    private final AtomicBoolean closed = new AtomicBoolean();

    private EffectsMcpServer(
            EffectsProtocolService protocol, InputStream input, OutputStream output) {
        this(protocol, input, output, new ConstrainedMcpJsonMapper());
    }

    EffectsMcpServer(
            EffectsProtocolService protocol,
            InputStream input,
            OutputStream output,
            ConstrainedMcpJsonMapper mapper) {
        this(protocol, input, output, mapper, MAX_RESPONSE_BYTES);
    }

    /**
     * Test seam mirroring {@link BoundedJsonRpcFramer}: shrinks the authoritative outbound
     * response cap without changing any production path.
     */
    EffectsMcpServer(
            EffectsProtocolService protocol,
            InputStream input,
        ...
        handler = new EffectsToolHandler(protocol, catalog);
        McpServer.AsyncSpecification<?> specification = McpServer.async(transport)
                .serverInfo("libgdx-agent-effects", currentVersion())
    ...
    public static EffectsMcpServer open(
            EffectsProtocolService protocol, InputStream input, OutputStream output) {
        return new EffectsMcpServer(
    ...
    private static String currentVersion() {
        String value = EffectsMcpServer.class.getPackage().getImplementationVersion();
    ...
    private static final class StdioProvider implements McpServerTransportProvider {
        ...
        private final ExecutorService readerExecutor =
                Executors.newSingleThreadExecutor(
                        Thread.ofVirtual().name("effects-mcp-input").factory());
        private final AtomicBoolean closed = new AtomicBoolean();
